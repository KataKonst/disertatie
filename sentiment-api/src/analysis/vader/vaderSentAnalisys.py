...
PUNC_LIST = [".", "!", "?", ",", ";", ":", "-", "'", "\"",
             "!!", "!!!", "??", "???", "?!?", "!?!", "?!?!", "!?!?"]
NEGATE = \
    ["nu", "niciuna", "niciunul", "nimic", "niciodata", "nicaieri", "nicidecum", "deloc", "nici", "nimeni"]
...
BOOSTER_DICT = \
    {"absolut": B_INCR, "incredibil": B_INCR, "dureros": B_INCR, "complet": B_INCR, "considerabil": B_INCR,
     ...
     "thoroughly": B_INCR, "totally": B_INCR, "tremendous": B_INCR,
     "uber": B_INCR, "de necrezut": B_INCR, "neobisnuit": B_INCR, "utterly": B_INCR,
     "foarte": B_INCR,"cel mai":B_INCR,
     "aproape": B_DECR, "barely": B_DECR, "cu greu": B_DECR, "just enough": B_DECR,
     "kind of": B_DECR, "kinda": B_DECR, "kindof": B_DECR, "kind-of": B_DECR,
     "less": B_DECR, "little": B_DECR, "marginally": B_DECR, "occasional": B_DECR, "partly": B_DECR,
     "scarcely": B_DECR, "slightly": B_DECR, "oarecum": B_DECR,
...
def scalar_inc_dec(word, first, valence, is_cap_diff):
    """
    ...
    if word_lower in BOOSTER_DICT:
        ...
        if word.isupper() and is_cap_diff:
            if valence > 0:
                ...
                scalar -= C_INCR
    elif (word+ " "+first).lower() in BOOSTER_DICT:

        if (word+ " "+first).lower() in BOOSTER_DICT:
            scalar = BOOSTER_DICT[(word+ " "+first).lower()]
            if valence < 0:
                scalar *= -1
            # check if booster/dampener word is in ALLCAPS (while others aren't)
            if word.isupper() and is_cap_diff:
...
class SentimentIntensityAnalyzer(object):
        ...
        if item_lowercase in self.lexicon:
            ...
            for start_i in range(0, 3):
                if i > start_i and words_and_emoticons[i - (start_i + 1)].lower() not in self.lexicon:
                    ...
                    first_word = words_and_emoticons[i - (start_i)];
                    s = scalar_inc_dec(words_and_emoticons[i - (start_i + 1)], first_word, valence, is_cap_diff)
    ...
    def _least_check(self, valence, words_and_emoticons, i):
        ...
        if i > 1 and words_and_emoticons[i - 1].lower() not in self.lexicon \
                and words_and_emoticons[i - 1].lower() == "putin":
            if words_and_emoticons[i - 2].lower() != "la" and words_and_emoticons[i - 2].lower() != "very":
    ...
    def _but_check(self, words_and_emoticons, sentiments):
        ...
        if 'dar' in words_and_emoticons or 'Dar' in words_and_emoticons:
            try:
                bi = words_and_emoticons.index('dar')
            except ValueError:
                bi = words_and_emoticons.index('Dar')
    ...
    def _never_check(self, valence, words_and_emoticons, start_i, i):
        if start_i == 0:
            if negated([words_and_emoticons[i - 1]]):
                valence = valence * N_SCALAR
        if start_i == 1:
            if words_and_emoticons[i - 2] == "niciodata" and \
                    (words_and_emoticons[i - 1] == "so" or
                     words_and_emoticons[i - 1] == "acest" or
                     words_and_emoticons[i - 1] == "aceasta"
                    ):
                valence = valence * 1.5
            elif negated([words_and_emoticons[i - (start_i + 1)]]):
                valence = valence * N_SCALAR
        if start_i == 2:
            if words_and_emoticons[i - 3] == "niciodata" and \
                    (words_and_emoticons[i - 2] == "aceasta" or words_and_emoticons[i - 2] == "aceasta") or \
                    (words_and_emoticons[i - 1] == "aceasta" or words_and_emoticons[i - 1] == "aceasta") or \
                    (words_and_emoticons[i - 2] == "acest" or words_and_emoticons[i - 2] == "acest") or \
                    (words_and_emoticons[i - 1] == "acest" or words_and_emoticons[i - 1] == "acest"):
